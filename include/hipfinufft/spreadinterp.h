// AUTO-GENERATED by tools/hipfinufft/cuda_to_hip.py from include/cufinufft/spreadinterp.h
...
template<typename T>
static constexpr __forceinline__ __device__ T hipFMA(const T a, const T b, const T c) {
...
constexpr __forceinline__ __host__ __device__ T fold_rescale(T x, int N) {
  ...
  if constexpr (std::is_same_v<T, float>) {
    // fused multiply-add, round to nearest even
    auto result = hipFMA(x, x2pi, half);
    // subtract, round down
   ...
   */
{
  ...
  const auto z = hipFMA(T(2), x, T(w - 1)); // scale so local grid offset z in [-1,1]
  // insert the auto-generated code which expects z, w args, writes to ker...
  if (upsampfac == 2.0) { // floating point equality is fine here
    using FLT = T;
#include "hipfinufft/contrib/ker_horner_allw_loop.inc"
  }
  if (upsampfac == 1.25) { // floating point equality is fine here
    using FLT = T;
#include "hipfinufft/contrib/ker_lowupsampfac_horner_allw_loop.inc"
  }
}

// Functions for calling different methods of spreading & interpolation
template<typename T> void cuspread1d(const hipfinufft_plan_t<T> &d_plan, int blksize);
template<typename T> void cuinterp1d(const hipfinufft_plan_t<T> &d_plan, int blksize);

template<typename T> void cuspread2d(const hipfinufft_plan_t<T> &d_plan, int blksize);
template<typename T> void cuinterp2d(const hipfinufft_plan_t<T> &d_plan, int blksize);

template<typename T> void cuspread3d(const hipfinufft_plan_t<T> &d_plan, int blksize);
template<typename T> void cuinterp3d(const hipfinufft_plan_t<T> &d_plan, int blksize);

// Wrappers for methods of spreading
template<typename T> void cuspread1d_prop(hipfinufft_plan_t<T> &d_plan);
template<typename T> void cuspread2d_prop(hipfinufft_plan_t<T> &d_plan);
template<typename T> void cuspread3d_prop(hipfinufft_plan_t<T> &d_plan);
